...
/*
- 신규 회원정보 등록 처리 요청과 응답 라우팅메소드
- 호출주소: http://localhost:3000/api/member/entry
- 호출방식: Post
** 중요1 클라이언트 호출하는 주소와 호출방식이 일치해야 해당 라우팅메소드가 실행됨
- 응답결과: 신규 회원 정보 등록처리후 DB에 저장된 회원정보 반환
*/
router.post('/entry', async (req, res) => {

    //백엔드 API로 호출하면 무조건 아래형식으로 데이터를 백엔드에서 반환한다.
    let apiResult = {
        code: 400, //요청상태코드: 200 정상, 400 요청리소스가 없음, 500 서버개발자 코딩 에러
    ...
    try {
        //로직 구현-로직이 에러가 나면 catch블럭으로 에러내용이 자동전달
        ...
        //위에 등록할 데이터가 db MEMBER테이블에 저장된 후 실제 저장된 회원이 다시 반환된다.
        let registedMember = await db.Member.create(member);
        registedMember.member_password = ""; //보안적이유로 암호는 프론트엔드에 전송 안 함.
        ...
        //Why? DB등록처리시 먼저 DB서버를 연결하는데 DB연결실패하면 연결에러메시지를 제공하는데 이런정보내에 보안적으로 공유하면 안되는
        //정보들이 존재
        apiResult.code = 500;
        apiResult.data = null;
...
- 호출방식: Post
- 응답결과: 사용자 메일/암호를 체크하고 JWT 사용자 인증토큰값을 프론트엔드에 반환한다.
*/
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (member) {
            ...
            //Step3: 사용자 암호값 일치여부를 체크한다.
            const compareResult = await encrypt.compare(password, member.member_password);

            if (compareResult) {
                //암호가 일치하는 경우

                //Step4: 사용자 메일주소/암호가 일치하는 경우 현재 로그인 사용자의 주요정보를 JSON데이터로 생성한다.
                const tokenJsonData = {
                ...
                //Step5: 인증된 사용자 json데이터를 JWT토큰 내에 담아 JWT 토큰문자열을 생성한다.
                //jwt.sign('토큰에 저장할 json데이터', 토큰화 할 때 사용하는 인증키값, 옵션값(토큰 유효기간 설정, 발급자))
                const token = await jwt.sign(tokenJsonData, process.env.JWT_AUTH_KEY, { expiresIn: '24h', issuer: "CBNU" });


                //Step6: JWT토큰 문자열을 프론트엔드로 반환한다.
                apiResult.code = 200;
...
router.get('/profile', async (req, res) => {
    ...
    try {
        ...
        //웹브라우저에서 전달되는 토큰값 예시: "Bearer huefralfukefrn"
        var token = req.headers.authorization.split('Bearer ')[1];

        //Step2: JWT 토큰 문자열내에서 인증사용자 JSON 데이터를 추출한다.
        //jwt.verify('토큰문자열', 토큰생성시사용한 인증키값) 실행 후 토큰내 저장된 json data를 반환함.
