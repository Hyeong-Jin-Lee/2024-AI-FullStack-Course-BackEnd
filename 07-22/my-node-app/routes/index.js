//노드 기본 백엔드 앱 공통적인 사용자 요청과 응답처리 전용 라우터파일
//해당 라우터 파일의 기본호출주소 : http://localhost:3000
//모든 라우터 파일은 기본 호출주소 체계를 app.js에서 사전 정의한다.
//express 객체 참조하기
var express = require('express');

//각종 사용자 요청과 응답을 처리할 수 있는 router객체를 express.Router()메소드를 통해 생성
//생성된 router객체를 통해 각종 사용자 요청과 응답을 처리
...
router.get('/', function(req, res, next) {
  ...
  console.log("서버측에서 웹브라우저 응답처리를 위한 res(HttpResponse)객체출력하기",res);
  ...
  res.render('index.ejs', { title: '충남대학교' });
});
...
router.get('/login',function(req,res){
  //서버에서 응답처리할 기능구현
  //views폴더내 login.ejs파일 내 html웹페이지를 웹브라우저에 전달
