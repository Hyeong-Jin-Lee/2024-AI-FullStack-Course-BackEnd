...
router.use('/sample',function(req,res,next){
  console.log('Index.js 라우터 파일 미들웨어 2호출',req.originalUrl);
  next();
...
- 호출방식: Get
- 응답결과: view/index.ejs 뷰파일 웹페이지 내용 응답결과로 제공
*/
...
//CheckParams 미들웨어를 요청이후 응답전에 먼저 실행하게되 하여 특정로직을 태운다.
// Step1:router.get메소드 실행-> checkParams()미들웨어함수실행-> 응답콜백함수실행
