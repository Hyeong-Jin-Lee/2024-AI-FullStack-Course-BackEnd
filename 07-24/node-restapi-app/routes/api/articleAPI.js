...
router.post('/create',async(req,res)=>{
    ...
    //백엔드 예외처리하기
    try{
        //Step1: 클라이언트에서 보내준 사용자 입력 json데이터를 추출
        ...
        //Step3: DB에 저장반환된 등록된 게시글 신규 게시글정보가 반환
        const dbArticle = {
            article_id:1,
            title,
            contents,
        ...
        //try{}블록스코프내에서 백엔드에러가 발생하면 catch(err){} 블럭으로 에러내용이 전달된다.
        apiResult.code=500;
        apiResult.data=null;
        apiResult.result="Server Error or 관리자에게 문의하세요.";
    }

        //Step4: DB에 저장되고 반환된 단일게시글 정보를 클라이언트에 반환
